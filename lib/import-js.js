 ...
 * @param {Function} thunk function returning a Promise
 * @return {Promise}
 */
function inCurrentWorkingDirectory(thunk) {
  ...
  return thunk().then((result) => {
    ...
    return result; // so that the promise can be chained
  }).catch(() => {
    process.chdir(originalCwd);
  });
}
...
function addImports(selections) {
  inCurrentWorkingDirectory(() => getImporter().addImports(selections))
...
function importWord() {
  ...
  inCurrentWorkingDirectory(() => getImporter().import(word))
    .then(handleImporterResult).catch(handleError);
}
...
function fixImports() {
  ...
  allowUnsafeNewFunction(() => {
    inCurrentWorkingDirectory(() => getImporter().fixImports())
      .then(handleImporterResult).catch(handleError);
  });
}
...
function gotoWord() {
  ...
  inCurrentWorkingDirectory(() => getImporter().goto(word)).then((result) => {
    const goto = path.join(currentFilePath()[0], result.goto);
    atom.open({ pathsToOpen: [goto], newWindow: false });
