 ...
 * @param {String} word
 */
function askForSelections(selectionsToAskFor, previousSelections, word) {
  ...
  if (selectionsToAskFor.length === 0) {
    ...
    importSelections(selections, word); // eslint-disable-line no-use-before-define
    return undefined;
  }
  ...
  askView.deferred
    ...
    .then((newSelections) => {
      askView.destroy();
      askForSelections(selectionsToAskFor, newSelections, word);
    });
 ...
 * @param {String} word
 */
function handleExecResult(result, word) {
  if (result.ask_for_selections) {
    askForSelections(result.ask_for_selections, [], word);
    return;
  }
...
function importWord() {
  ...
  exec(['--word', word])
    .then((result) => handleExecResult(result, word))
 ...
 * @param {String} word
 */
function importSelections(selections, word) {
  // Foo:0,Bar:1
